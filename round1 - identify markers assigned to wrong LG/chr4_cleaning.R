...
#ggplot(rf_chr.df, aes(x = index, y = median)) +
#  geom_point() +
#  geom_text(aes(label = as.character(index)), size = 10)
...
rf_chr.df$outliers <- 0
rf_chr.df$outliers[c(41, 43, 47, 50, 58, 60,
                     61, 63, 64, 66, 74, 97,
                     116, 133, 134)] <- 1
...
plotRF(cross20.chr.rmOutliers.rmRM, what = "rf", main = "RF plot - only rf (not LOD scores)")
dev.off()
