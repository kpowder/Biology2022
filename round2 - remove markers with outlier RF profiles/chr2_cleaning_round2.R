...
rf_chr.df$outliers[SlidingWindow.outliers(rf_chr.df$median, win = 12, steps = 3)] <- 1
...
rf_chr.df$outliers <- 0
rf_chr.df$outliers[c(3, 16, 26, 34, 49, 81, 85, 97, 98, 100, 103, 104, 
                     106, 107, 113, 118, 121, 132, 134, 140, 145, 153,
                     166, 169, 170, 171, 172, 179, 181, 188, 193, 200, 204)] <- 1
...
chr_info(cross20.chr.rmOutliers.rmRM, chr = chrnames(cross)[chr]) %>% 
  ggplot(aes(x=bp, y=pos)) + 
  geom_point() +
  theme_light()
chr_info(cross20.chr.rmOutliers.rmRM, chr = chrnames(cross)[chr]) %>% 
  plot_ly(x=~bp, y=~pos)
