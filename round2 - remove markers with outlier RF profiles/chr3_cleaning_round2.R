...
#load("../cross20.chr2.rmOutliers.rmRM.Rdata")
...
rf_chr.df$outliers[SlidingWindow.outliers(rf_chr.df$median, win = 12, steps = 2)] <- 1

ggplot(rf_chr.df, aes(x = index, y = median)) +
  geom_point() +
  geom_text(aes(label = as.character(index)), size = 3)

ggplot(rf_chr.df, aes(x = index, y = median)) +
  geom_point() +
  geom_text(aes(label = as.character(index)), size = 3, hjust = 2)

tmp.chr_info <- chr_info(cross20.chr, chr = chrnames(cross)[chr]) %>% 
  mutate(index = 1:nrow(.))

tmp.chr_info_round1 <- chr_info(cross.filtered.round1, chr = chrnames(cross)[chr]) %>% 
  mutate(index = 1:nrow(.))

View(tmp.chr_info); View(tmp.chr_info_round1)
...
# or don't agree)
#SlidingWindow.outliers(rf_chr.df$median, win = 12, steps = 3)
rf_chr.df$outliers <- 0
rf_chr.df$outliers[c(1, 2, 3, 4, 5, 11, 12, 13, 14, 16, 18, 20, 23, 
                     24, 25, 26, 27, 28, 29, 30, 42, 54, 58, 
                     67, 88, 97, 104, 121, 123, 125, 141, 142, 148, 
                     149, 154, 155, 157, 160, 162, 170, 188, 193, 199, 
...
#write.table(tmp.out,
#            file = paste("all_outlier_markers_chr", chr, "_round2.txt", sep = ""),
#            quote = FALSE, sep = "\t",
...
newmap <- est.map(cross20.chr.rmOutliers, 
                  error.prob=0.005, 
                  offset = 0)
...
# plot Map
pdf(paste("map_plots_chr", chr, "_round2.1.pdf", sep = ""))
par(mfrow=c(1,2))
...
# estimate map in chr without RM
...
write.table(tmp.out,
            file = paste("redundant_markers_chr", chr, "_round2.1.txt", sep = ""),
            quote = FALSE, sep = "\t",
            row.names = FALSE, col.names = TRUE)

# generate summary plots
pdf(paste("summary_plots_chr", chr, "_round2.1.pdf", sep = ""))
par(mfrow=c(1,1))
...
plotRF(cross20.chr.rmOutliers.rmRM, what = "lod")
chr_info(cross20.chr.rmOutliers.rmRM, chr = chrnames(cross)[chr]) %>% ggplot(aes(x=bp, y=pos)) + geom_point()
#chr_info(cross20.chr.rmOutliers.rmRM, chr = chrnames(cross)[chr]) %>% plot_ly(x=~bp, y=~pos)
dev.off()

# saving chr info after removing outliers and RM in a cross object
save(cross20.chr.rmOutliers.rmRM, file = paste("cross20.chr", chr, ".rmOutliers.rmRM.round2.1.Rdata", sep = ""))
