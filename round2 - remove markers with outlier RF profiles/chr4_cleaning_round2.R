...
load("../cross20.chr4.rmOutliers.rmRM.Rdata")
...
rf_chr.df$outliers[SlidingWindow.outliers(rf_chr.df$median, win = 10, steps = 3)] <- 1

#ggplot(rf_chr.df, aes(x = index, y = median)) +
#  geom_point() +
#  geom_text(aes(label = ifelse(outliers==1, as.character(index),'')), size = 6)
...
rf_chr.df$outliers <- 0
rf_chr.df$outliers[c(9, 14, 15, 26, 27, 29, 31, 38, 41, 43, 47, 
                     50, 54, 58, 60, 61, 63, 64, 66, 74, 76, 82, 88, 
                     90, 97, 100, 103, 111, 113, 116, 126, 133, 134,
                     144, 146)] <- 1
...
print("This are the detected misassembled markers:")
...
# plot Map
#par(mfrow=c(1,2))
pdf(paste("output_plots_chr", chr, ".pdf", sep = ""))
