...
load("../cross20.chr7.rmOutliers.rmRM.Rdata")
...
tmp.chr_info_round1 <- chr_info(cross.filtered, chr = chrnames(cross)[chr]) %>% 
  mutate(index = 1:nrow(.))
...
rf_chr.df$outliers <- 0
rf_chr.df$outliers[c(2, 3, 7, 9, 12, 15, 23, 29, 36, 41, 44, 46, 48, 
                     55, 74, 79, 82, 85, 92, 96, 111, 125, 131, 132, 
                     140, 142, 147, 155, 161, 178, 183, 186, 193, 195,
                     201, 203, 215, 230, 252, 253, 257, 259, 260, 271,
                     276, 277, 280, 282, 284, 287, 301, 307, 308, 322, 
                     324, 325, 329, 333, 334, 343, 344, 345, 350, 353, 
                     354, 362, 364, 368, 369, 371, 376, 377, 380, 383, 
                     388, 389)] <- 1
...
write.table(tmp.out,
            file = paste("redundant_markers_chr", chr, "_round2.txt", sep = ""),
            quote = FALSE, sep = "\t",
...
chr_info(cross20.chr.rmOutliers, chr = chrnames(cross)[chr]) %>% plot_ly(x=~bp, y=~pos)
...
save(cross20.chr.rmOutliers.rmRM, file = paste("cross20.chr", chr, ".rmOutliers.rmRM.round2.Rdata", sep = ""))
